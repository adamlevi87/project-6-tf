...
output "argocd_dashboard_configmap_name" {
  ...
  value       = kubernetes_config_map.argocd_dashboard.metadata[0].name
}

output "dashboard_configmaps" {
  description = "List of all created dashboard ConfigMaps"
  value = compact([
    kubernetes_config_map.argocd_dashboard.metadata[0].name,
    try(kubernetes_config_map.aws_lbc_dashboard[0].metadata[0].name, null),
    try(kubernetes_config_map.eks_overview_dashboard[0].metadata[0].name, null)
  ])
}
